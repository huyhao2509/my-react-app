...
import { getGenres, getPopularMovies } from "../services/MoviesService";
...
export default function Popular() {
  ...
  const handlePageChange = (nextPage: number) => {
    ...
    navigate(`/popular?page=${normalizedPage}`);
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [movieData, genreData] = await Promise.all([getPopularMovies(page), getGenres()]);
      ...
      <div className="container min-h-screen px-4 py-8 md:px-6">
        <div className="mb-6 fade-up">
          <div className="inline-flex items-center gap-2 rounded-full border border-cyan-300/40 bg-cyan-400/10 px-3 py-1 text-xs font-semibold tracking-[0.15em] text-cyan-200">
            <Sparkles size={14} /> TRENDING BOARD
          </div>
          <h1 className="mt-3 text-3xl font-extrabold text-gradient md:text-4xl">Popular Movies</h1>
        </div>
        <MoviesGridSkeleton />
      </div>
    );
  ...
  return (
    <div className="min-h-screen">
      ...
      <MoviesList title="Popular Movies" data={movies} genres={genres} singleRow />
